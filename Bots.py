    ...
    #Should calculate how many unit one should use in attack.
        ...
        while self.moveable.__len__() > 0:
            if r.random() > 0.01:
                ...
                if toTile.owner != self.currentPlayer:
                    moved[toTile.cords.__str__()] = pos
                    allUnit = self.findMovableInTile(pos)
                    if allUnit.__len__() > toTile.units.__len__():
                        for unitA in allUnit:
                            self.moveUnit(self.map.board[pos[0]][pos[1]], toTile, 1, unit.__class__, unitA)
                else:
                    self.moveUnit(self.map.board[pos[0]][pos[1]], toTile, 1, unit.__class__, unit)
            else:
                break
